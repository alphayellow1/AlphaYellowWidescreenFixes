...
int main()
{
    ...
    cout << "Elite Forces: WWII Normandy (2001) FOV Fixer v1.4 by AlphaYellow, 2024\n\n----------------\n";
    ...
    do
    {
        ...
        while (fileNotFound)
        {
            cout << "\nFailed to open client.exe, check if the executable has special permissions allowed that prevent the fixer from opening it (e.g: read-only mode), it's not present in the same directory as the fixer, or if the executable is currently running. Press Enter when all the mentioned problems are solved." << endl;
            cin.get();
            ...
            if (file.is_open())
            {
                if (fileOpened == 0)
                {
                    cout << "\nclient.exe opened successfully!" << endl;
