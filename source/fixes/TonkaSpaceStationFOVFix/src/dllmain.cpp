...
// Fix details
std::string sFixName = "TonkaSpaceStationFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::TSS, {"Tonka Space Station", "SpaceStation.exe"}},
};
...
{
	fNewCameraFOV = fOriginalCameraFOV * (1.0f / fFOVFactor);
...
void FOVFix()
{
	if (eGameType == Game::TSS && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		std::uint8_t* CameraHFOVInstructionScanResult = Memory::PatternScan(exeModule, "D8 4A 34 D9 42 30 D8 62 2C");
		...
		if (CameraFOVInstructionScanResult)
		{
			...
			Memory::PatchBytes(CameraFOVInstructionScanResult + 0x4, "\x90\x90\x90\x90\x90\x90", 6);

			CameraFOVInstructionHook = safetyhook::create_mid(CameraFOVInstructionScanResult + 0xA, CameraFOVInstructionMidHook);
		}
		else
		{
