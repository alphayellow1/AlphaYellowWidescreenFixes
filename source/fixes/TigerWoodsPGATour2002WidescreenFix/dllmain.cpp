...
// Fix details
std::string sFixName = "TigerWoodsPGATour2002WidescreenFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::TWPGAT2002, {"Tiger Woods PGA Tour 2002", "TW2002.exe"}},
};
...
void WidescreenFix()
{
	if (eGameType == Game::TWPGAT2002 && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		// Located in DXWrap.dll.CDisplay::FindMode
		std::uint8_t* ResolutionListScanResult = Memory::PatternScan(dllModule2, "C7 44 24 14 20 03 00 00 C7 44 24 10 58 02 00 00 EB 34 C7 44 24 14 00 04 00 00 C7 44 24 10 00 03 00 00 EB 22 C7 44 24 14 00 05 00 00 C7 44 24 10 00 04 00 00");
		if (ResolutionListScanResult)
		{
			spdlog::info("Resolution List Scan: Address is DXWrap.dll+{:x}", ResolutionListScanResult - (std::uint8_t*)dllModule2);
			...
			// 1024x768
			Memory::Write(ResolutionListScanResult + 22, iCurrentResX);
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(dllModule2, "89 96 ?? ?? ?? ?? D9 1C");
