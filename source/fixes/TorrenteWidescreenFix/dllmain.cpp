...
static void HandleModule(HMODULE moduleHandle, const std::wstring& baseName, bool loaded)
{
	...
	if (bFixActive == true && eGameType == Game::TORRENTE)
	{
		...
		else if (name == L"contadorsalud.dll")
		{
			...
			if (loaded)
			{
				std::vector<std::uint8_t*> ContadorSaludHUDInstructionsScansResult = Memory::PatternScan(dllModule9, "68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 ?? 50 8D 4C 24 ?? 51 68", "68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 4C 24 ?? 51 8D 54 24 ?? 52 68", "68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 ?? 50 8D 4C 24 ?? 51 6A", "68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 54 24 ?? 52 8D 44 24 ?? 50 6A", "68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 54 24 ?? 52 8D 44 24 ?? 50 68", "68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 4C 24 ?? 51 8D 54 24 ?? 52 6A", "68 ?? ?? ?? ?? 8D 4C 24 ?? 51 8D 54 24 ?? 2B C6", "68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 4C 24 ?? 51 8D 54 24 ?? 52 56");
