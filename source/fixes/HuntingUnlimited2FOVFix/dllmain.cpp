...
void FOVFix()
{
	if (eGameType == Game::HU2 && bFixActive == true)
	{
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(dllModule2, "8B 40 5C 8D 4C 24 30 51 89 44 24 3C FF 15 ?? ?? ?? ??");
		...
		if (WeaponFOVInstructionScanResult)
		{
			...
			{
				float& fCurrentWeaponFOV = Memory::ReadMem(ctx.ebp + 0x2B0);
