...
// Fix details
std::string sFixName = "CombatWingsFOVFix";
...
void FOVFix()
{
	if (eGameType == Game::CW && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* CameraFOVInstructionsScanResult = Memory::PatternScan(exeModule, "89 3E 8B 7A ?? 89 7E ?? 8B 7A");
		if (CameraFOVInstructionsScanResult)
		{
			...
			{
				float fCurrentCameraHFOV = std::bit_cast<float>(ctx.edi);

				if (fCurrentCameraHFOV == 1.777777791f || fCurrentCameraHFOV == 0.169885397f)
				{
			...
			Memory::PatchBytes(CameraFOVInstructionsScanResult + 5, "\x90\x90\x90", 3);

			CameraVFOVInstructionHook = safetyhook::create_mid(CameraFOVInstructionsScanResult + 5, [](SafetyHookContext& ctx)
			{
				float fCurrentCameraVFOV = std::bit_cast<float>(ctx.edi);

				if (fCurrentCameraVFOV == 1.333333373f || fCurrentCameraVFOV == 0.1274140477f)
				{
