...
double oldWidth = 4.0, oldHeight = 3.0, oldHFOV, oldAspectRatio = oldWidth / oldHeight, newAspectRatio;
...
void OpenFile(fstream &file)
{
    ...
    file.open("Razor.exe", ios::in | ios::out | ios::binary);
    ...
    while (fileNotFound)
    {
        ...
        if (!file.is_open())
        {
            cout << "\nFailed to open Razor.exe, check if the executable has special permissions allowed that prevent the fixer from opening it (e.g: read-only mode), it's not present in the same directory as the fixer, or if the executable is currently running. Press Enter when all the mentioned problems are solved." << endl;
            do
            {
        ...
        {
            cout << "\nRazor.exe opened successfully!" << endl;
