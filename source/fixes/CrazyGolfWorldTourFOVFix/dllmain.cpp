...
class CrazyGolfWorldTourFix final : public FixBase
{
public:
	explicit CrazyGolfWorldTourFix(HMODULE selfModule) : FixBase(selfModule)
	{
		s_instance_ = this;
	}

	~CrazyGolfWorldTourFix() override
	{
		if (s_instance_ == this)
		{
			s_instance_ = nullptr;
...
protected:
	...
	{
		return "CrazyGolfWorldTourFOVFix";
	}
	...
	{
		return Util::stringcmp_caseless(exeName, "CrazyGolfWorldTour.exe");
	}
...
private:
	...
	void GetResolutionFromIni()
	{
		...
		std::ifstream iniFile("CrazyGolfWorldTour.ini");
	...
	inline static CrazyGolfWorldTourFix* s_instance_ = nullptr;
};

static std::unique_ptr<CrazyGolfWorldTourFix> g_fix;
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		g_fix = std::make_unique<CrazyGolfWorldTourFix>(hModule);
		g_fix->Start();
		break;
	}
