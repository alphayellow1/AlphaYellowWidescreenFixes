...
// Fix details
std::string sFixName = "LuxuryLinerTycoonWidescreenFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::LTT, {"Luxury Liner Tycoon", "Cruise.exe"}},
};
...
void SetARAndFOV()
{
	std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "68 ?? ?? ?? ?? 51 8B 4C 24 ?? DD D8");
	...
	std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "DD 05 ?? ?? ?? ?? D9 F2 68");
...
void WidescreenFix()
{
	if (eGameType == Game::LTT && bFixActive == true)
	{
		std::vector<std::uint8_t*> ResolutionInstructionsScansResult = Memory::PatternScan(exeModule, "81 F9 ?? ?? ?? ?? 0F 8E", "8B 0F 89 0D ?? ?? ?? ?? 8B 57");
		if (Memory::AreAllSignaturesValid(ResolutionInstructionsScansResult) == true)
		{
			...
			{
				int& iCurrentWidth = Memory::ReadMem(ctx.edi);

				int& iCurrentHeight = Memory::ReadMem(ctx.edi + 0x4);
