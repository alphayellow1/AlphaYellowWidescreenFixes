...
// Fix details
std::string sFixName = "CabelasBigGameHunter2008TrophyBucksFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::CBGH2008TB, {"Cabela's Big Game Hunter 2008 Trophy Bucks", "Ctb.exe"}},
};
...
bool DetectGame()
{
	...
	while ((dllModule3 = GetModuleHandleA("GameDll8r.dll")) == nullptr)
	{
		spdlog::warn("GameDll8r.dll not loaded yet. Waiting...");
		Sleep(100);
	}

	spdlog::info("Successfully obtained handle for GameDll8r.dll: 0x{:X}", reinterpret_cast<uintptr_t>(dllModule3));
...
void CameraFOVInstructionMidHook(SafetyHookContext& ctx)
{
	...
	if (fCurrentCameraFOV == 60.0f || fCurrentCameraFOV == 59.98499298f)
	{
...
void FOVFix()
{
	if (eGameType == Game::CBGH2008TB && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(dllModule2, "D9 87 D0 00 00 00 DC 0D ?? ?? ?? ??");
		...
		std::uint8_t* AspectRatioInstruction1ScanResult = Memory::PatternScan(dllModule2, "D8 8F D4 00 00 00 D8 B7 D8 00 00 00 D9 9F E0 00 00 00");
		...
		std::uint8_t* AspectRatioInstruction2ScanResult = Memory::PatternScan(dllModule2, "D8 B7 D8 00 00 00 D9 9F E0 00 00 00 D9 EE D8 97 EC 00 00 00 DF E0");
