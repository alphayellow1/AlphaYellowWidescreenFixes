	...
	if (eGameType == Game::CBGH2008TB && bFixActive == true)
	{
		...
		if (CameraFOVInstructionScanResult)
		{
			...
			CameraFOVInstructionMidHook = safetyhook::create_mid(CameraFOVInstructionScanResult, [](SafetyHookContext& ctx)
			{
				...
				if (fCurrentFOVValue != fLastModifiedFOV && fCurrentFOVValue != CalculateNewFOV(60.0f))
				{
		...
		{
			spdlog::info("Aspect Ratio Instruction 2: Address is EngineDll8r.dll+{:x}", AspectRatioInstruction2ScanResult + 6 - (std::uint8_t*)dllModule2);
		...
		{
			spdlog::error("Failed to locate third aspect ratio instruction memory address.");
			return;
		}
