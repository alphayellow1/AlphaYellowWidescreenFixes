...
void FOVFix()
{
	if (eGameType == Game::CCK4 && bFixActive == true)
	{
		...
		if (InARaceTriggerInstructionScanResult)
		{
			...
			uint8_t* TriggerValueAddress = Memory::GetPointerFromAddress(InARaceTriggerInstructionScanResult + 1, Memory::PointerMode::Absolute);
		...
		if (CameraHFOVInstructionScanResult)
		{
			...
			{
				float& fCurrentCameraHFOV = Memory::ReadMem(ctx.eax + 0x310);
		...
		if (CameraVFOVInstructionScanResult)
		{
			...
			{
				float& fCurrentCameraVFOV = Memory::ReadMem(ctx.eax + 0x314);	
