...
// Fix details
std::string sFixName = "DonkeyXoteFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::DX, {"Donkey Xote", "Donkey Xote.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::DX && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "D8 0D ?? ?? ?? ?? 89 44 24 ?? D9 5C 24 ?? EB");
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "DD 05 ?? ?? ?? ?? 55 D9 F2");
