...
// Fix details
std::string sFixName = "ProjectEarthStarmageddonFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::PES, {"Project Earth: Starmageddon", "projectearth.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::PES && bFixActive == true)
	{
		...
		if (CameraHFOVInstructionScanResult)
		{
			...
			{
				float& fCurrentCameraHFOV = Memory::ReadMem(ctx.esi + 0x138);
		...
		std::uint8_t* CameraFOVScanResult = Memory::PatternScan(exeModule, "C7 44 24 10 00 00 80 3F D9 5C 24 08 D9 05 C4 E5 58 00");
		if (CameraFOVScanResult)
		{
			spdlog::info("Camera FOV: Address is {:s}+{:x}", sExeName.c_str(), CameraFOVScanResult + 4 - (std::uint8_t*)exeModule);
