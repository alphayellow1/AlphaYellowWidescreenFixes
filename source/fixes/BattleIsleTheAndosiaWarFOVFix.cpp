...
const streampos kFOVOffset = 0x001E2626;
...
void OpenFile(fstream &file)
{
    ...
    file.open("bitaw.exe", ios::in | ios::out | ios::binary);
    ...
    while (fileNotFound)
    {
        ...
        if (!file.is_open())
        {
            cout << "\nFailed to open bitaw.exe, check if the executable has special permissions allowed that prevent the fixer from opening it (e.g: read-only mode), it's not present in the same directory as the fixer, or if the executable is currently running. Press Enter when all the mentioned problems are solved." << endl;
            do
            {
        ...
        {
            cout << "\nbitaw.exe opened successfully!" << endl;
