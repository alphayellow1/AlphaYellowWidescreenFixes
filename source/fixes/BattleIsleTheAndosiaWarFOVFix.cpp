...
const streampos kCameraFOVOffset = 0x001E2626;
...
float currentCameraFOVInRadians, currentCameraFOVInDegrees, newCameraFOVInDegrees, newCameraFOVInDegreesValue, newCameraFOVInRadians, oldWidth = 4.0f, oldHeight = 3.0f, oldHorizontalFOV = 90.0f, oldAspectRatio = oldWidth / oldHeight;
char ch;
...
int main()
{
    ...
    cout << "Battle Isle: The Andosia War (2000) FOV Fixer v1.0 by AlphaYellow, 2024\n\n----------------\n";

    do
    {
        OpenFile(file, "bitaw.exe");

        file.seekg(kCameraFOVOffset);
        file.read(reinterpret_cast<char *>(&currentCameraFOVInRadians), sizeof(currentCameraFOVInRadians));

        // Converts the field of view value from radians to degrees
        currentCameraFOVInDegrees = RadToDeg(currentCameraFOVInRadians);

        cout << "\nCurrent camera FOV is " << currentCameraFOVInDegrees << "\u00B0" << endl;

        cout << "\n- Do you want to set the camera FOV automatically based on the desired resolution (1) or set a custom camera FOV value (2)?: ";
        ...
        case 2:
            cout << "\n- Enter the desired camera FOV value (from 1\u00B0 to 180\u00B0, default field of view for 4:3 aspect ratio is 90\u00B0): ";
        ...
        if (file.good())
        {
            ...
            cout << "\nSuccessfully changed " << descriptor << " the camera FOV to " << newCameraFOVInDegrees << "\u00B0." << endl;
        }
        else
        {
