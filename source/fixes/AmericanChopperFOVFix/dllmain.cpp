...
// Fix details
std::string sFixName = "AmericanChopperFOVFix";
...
enum CameraFOVInstructionIndices
{
	HFOVScan,
	VFOVScan
};
...
const std::map<Game, GameInfo> kGames = {
	{Game::AC, {"American Chopper", "Default.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::AC && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::vector<std::uint8_t*> CameraFOVInstructionsScansResult = Memory::PatternScan(exeModule, "D9 44 24 ?? 56 D8 0D", "D9 44 24 ?? D8 0D ?? ?? ?? ?? D9 96");
		if (Memory::AreAllSignaturesValid(CameraFOVInstructionsScansResult) == true)
		{
			...
			{
				float& fCurrentCameraHFOV = *reinterpret_cast<float*>(ctx.esp + 0x28);
			...
			Memory::WriteNOPs(CameraFOVInstructionsScansResult[VFOVScan], 4);

			CameraVFOVInstructionHook = safetyhook::create_mid(CameraFOVInstructionsScansResult[VFOVScan], [](SafetyHookContext& ctx)
			{
				float& fCurrentCameraVFOV = *reinterpret_cast<float*>(ctx.esp + 0x30);
