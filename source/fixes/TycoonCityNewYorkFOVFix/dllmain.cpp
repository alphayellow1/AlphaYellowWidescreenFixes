...
// Fix details
std::string sFixName = "TycoonCityNewYorkFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::TCNY, {"Tycoon City: New York", "TycoonCity.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::TCNY && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "D9 81 1C 01 00 00 C3 CC CC CC CC CC CC CC CC CC");
		if (AspectRatioInstructionScanResult)
		{
			...
			{
				float& fCurrentAspectRatio = *reinterpret_cast<float*>(ctx.ecx + 0x11C);
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "D9 81 18 01 00 00 C3 CC CC CC CC CC CC CC CC CC");
		if (CameraFOVInstructionScanResult)
		{
			...
			{
				// Store a float reference to the current FOV value located in the [ECX+118] memory address
				float& fCurrentCameraFOV = *reinterpret_cast<float*>(ctx.ecx + 0x118);
