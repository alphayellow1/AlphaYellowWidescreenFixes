...
void WidescreenFix()
{
	if (eGameType == Game::JACKED && bFixActive == true)
	{
		...
		if (AspectRatioInstructionScanResult)
		{
			...
			{
				float& fCurrentAspectRatio = Memory::ReadMem(ctx.eax);
		...
		if (CameraFOVInstructionsScanResult)
		{
			...
			CameraHFOVAddress = Memory::GetPointerFromAddress(CameraFOVInstructionsScanResult + 24, Memory::PointerMode::Absolute);
