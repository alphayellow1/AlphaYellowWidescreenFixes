...
// Fix details
std::string sFixName = "WingsOfHonourFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::WOH, {"Wings of Honour", "woh.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::WOH && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "D9 40 ?? D8 48 ?? D8 48");
		if (AspectRatioInstructionScanResult)
		{
			...
			{
				float& fCurrentAspectRatio = Memory::ReadMem(ctx.eax + 0x58);
