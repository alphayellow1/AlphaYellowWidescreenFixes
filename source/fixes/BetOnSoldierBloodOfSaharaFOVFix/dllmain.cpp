...
void FOVFix()
{
	if ((eGameType == Game::BOSBOS || eGameType == Game::BOSBOSGAME) && bFixActive == true)
	{
		...
		if (Memory::AreAllSignaturesValid(CameraFOVInstructionsScansResult) == true)
		{
			...
			spdlog::info("Hipfire & Zoom Camera FOV Instructions Scan: Address is Bos.dll+{:x}", CameraFOVInstructionsScansResult[HipfireAndZoomFOVScan] - (std::uint8_t*)dllModule3);
