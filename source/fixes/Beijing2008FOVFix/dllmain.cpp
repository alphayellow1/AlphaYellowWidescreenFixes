...
// Fix details
std::string sFixName = "Beijing2008FOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::BEIJING2008, {"Beijing 2008", "Beijing.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::BEIJING2008 && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* AspectRatioInstructionsScanResult = Memory::PatternScan(exeModule, "D9 05 ?? ?? ?? ?? EB 06 D9 05 ?? ?? ?? ?? 8B 44 24 08 D9 5C 24 04");
		if (AspectRatioInstructionsScanResult)
		{
			...
			AspectRatioInstruction1Hook = safetyhook::create_mid(AspectRatioInstructionsScanResult, AspectRatioInstruction1MidHook);

			Memory::PatchBytes(AspectRatioInstructionsScanResult + 8, "\x90\x90\x90\x90\x90\x90", 6);

			AspectRatioInstruction2Hook = safetyhook::create_mid(AspectRatioInstructionsScanResult + 8, AspectRatioInstruction2MidHook);
		}
		else
		{
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "D9 44 24 04 D9 11 DC 0D ?? ?? ?? ?? DC 0D ?? ?? ?? ?? D9 5C 24 04 D9 44 24 04");
