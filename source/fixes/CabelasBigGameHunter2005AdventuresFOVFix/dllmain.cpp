...
// Fix details
std::string sFixName = "CabelasBigGameHunter2005AdventuresFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::CBGH2005A, {"Cabela's Big Game Hunter 2005 Adventures", "Bgh2005.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::CBGH2005A && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* InsideCarTriggerInstructionScanResult = Memory::PatternScan(dllModule3, "0F 94 C0 A3 ?? ?? ?? ?? C3 90 90 90 90 90 90 90 90 90 90 90 90 90 90");
		if (InsideCarTriggerInstructionScanResult)
		{
			spdlog::info("Inside Car Trigger Instruction: Address is GameDll6.dll+{:x}", InsideCarTriggerInstructionScanResult + 3 - (std::uint8_t*)dllModule3);

			static SafetyHookMid InsideCarTriggerInstructionMidHook{};

			InsideCarTriggerInstructionMidHook = safetyhook::create_mid(InsideCarTriggerInstructionScanResult + 3, [](SafetyHookContext& ctx)
			{
				iInsideCar = std::bit_cast<uint32_t>(ctx.eax);
			});
		}
		else
		{
		...
		{
			spdlog::info("Aspect Ratio Instruction 1: Address is EngineDll6.dll+{:x}", AspectRatioInstruction1ScanResult + 0x6 - (std::uint8_t*)dllModule2);
		...
		{
			spdlog::info("Aspect Ratio Instruction 2: Address is EngineDll6.dll+{:x}", AspectRatioInstruction2ScanResult - (std::uint8_t*)dllModule2);
