...
// Fix details
std::string sFixName = "SaltLake2002FOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::SL2002, {"Salt Lake 2002", "SaltLake2002.exe"}},
};
...
{
	float& fCurrentCameraFOV1 = *reinterpret_cast<float*>(ctx.eax + 0xCC);
...
{
	float& fCurrentCameraFOV2 = *reinterpret_cast<float*>(ctx.edx + 0xCC);
...
{
	float& fCurrentCameraFOV3 = *reinterpret_cast<float*>(ctx.esi + 0xCC);
...
void FOVFix()
{
	if (eGameType == Game::SL2002 && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "DB 44 24 1C 89 54 24 1C 8B 47 1C 83 EC 08 8D 4E 1C DA 74 24 24 D9 5C 24 04");
		...
		std::uint8_t* CameraFOVInstruction1ScanResult = Memory::PatternScan(exeModule, "D9 80 CC 00 00 00 D9 1C 24 51 B9 ?? ?? ?? ?? E8 ?? ?? ?? ?? F6 C3 08");
		...
		std::uint8_t* CameraFOVInstruction2ScanResult = Memory::PatternScan(exeModule, "D9 82 CC 00 00 00 D8 2D ?? ?? ?? ?? D8 0D ?? ?? ?? ?? D9 54 24 18 D9 5C 24 1C D9 44 24 18 D9 FE");
		...
		std::uint8_t* CameraFOVInstruction3ScanResult = Memory::PatternScan(exeModule, "D9 82 CC 00 00 00 D8 2D ?? ?? ?? ?? D8 0D ?? ?? ?? ?? D9 54 24 18 D9 5C 24 1C D9 44 24 18 D9 FE");
