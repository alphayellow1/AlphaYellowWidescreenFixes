...
void FOVFix()
{
	if (eGameType == Game::RQ && bFixActive == true)
	{
		...
		if (CameraVFOVInstructionScanResult)
		{
			...
			Memory::PatchBytes(CameraVFOVInstructionScanResult + 0x12, "\x90\x90\x90\x90\x90\x90", 6);

			CameraVFOVInstructionHook = safetyhook::create_mid(CameraVFOVInstructionScanResult + 0x18, CameraVFOVInstructionMidHook);
		}
		else
		{
