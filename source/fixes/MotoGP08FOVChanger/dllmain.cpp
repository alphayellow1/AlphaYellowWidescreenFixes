...
// Fix details
std::string sFixName = "MotoGP08FOVChanger";
...
const std::map<Game, GameInfo> kGames = {
	{Game::MG08, {"MotoGP 08", "Launcher.exe"}},
};
...
void FOVChanger()
{
	if (eGameType == Game::MG08 && bFixActive == true)
	{
		std::vector<std::uint8_t*> CameraFOVInstructionsScansResult = Memory::PatternScan(exeModule, "D8 47 ?? D9 1F", "D9 03 DC 0D", "D9 07 DC 0D");
		if (Memory::AreAllSignaturesValid(CameraFOVInstructionsScansResult) == true)
		{
			...
			{
				CameraFOVInstructionsMidHook(ctx.edi + 0x4, FADD);
			});

			Memory::WriteNOPs(CameraFOVInstructionsScansResult[FOV2], 2);

			CameraFOVInstruction2Hook = safetyhook::create_mid(CameraFOVInstructionsScansResult[FOV2], [](SafetyHookContext& ctx)
			{
				CameraFOVInstructionsMidHook(ctx.ebx, FLD);
			});

			Memory::WriteNOPs(CameraFOVInstructionsScansResult[FOV3], 2);

			CameraFOVInstruction3Hook = safetyhook::create_mid(CameraFOVInstructionsScansResult[FOV3], [](SafetyHookContext& ctx)
			{
				CameraFOVInstructionsMidHook(ctx.edi, FLD);
			});
		}
	}
}
