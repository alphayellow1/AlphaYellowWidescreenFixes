	...
	if (eGameType == Game::HDRTTR && bFixActive == true)
	{
		...
		if (CameraFOVInstructionScanResult)
		{
			...
			CameraFOVInstructionMidHook = safetyhook::create_mid(CameraFOVInstructionScanResult, [](SafetyHookContext& ctx)
			{
				...
				if (currentFOVValue == 50.0f || currentFOVValue == 75.0f)
				{
				...
				if (currentFOVValue != lastModifiedFOV && currentFOVValue != fFOVFactor * (2.0f * RadToDeg(atanf(tanf(DegToRad(50.0f / 2.0f)) * (fNewAspectRatio / fOldAspectRatio)))) && currentFOVValue != fFOVFactor * (2.0f * RadToDeg(atanf(tanf(DegToRad(75.0f / 2.0f)) * (fNewAspectRatio / fOldAspectRatio)))))
				{
