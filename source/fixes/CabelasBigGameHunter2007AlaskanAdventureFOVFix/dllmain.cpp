...
const std::map<Game, GameInfo> kGames = {
	{Game::CBGH2007AA, {"Cabela's Big Game Hunter 2007 Alaskan Adventure", "BGH10.exe"}},
};
...
bool DetectGame()
{
	...
	while ((dllModule2 = GetModuleHandleA("EngineDll7.dll")) == nullptr)
	{
		spdlog::warn("EngineDll7.dll not loaded yet. Waiting...");
		Sleep(100);
	}

	spdlog::info("Successfully obtained handle for EngineDll7.dll: 0x{:X}", reinterpret_cast<uintptr_t>(dllModule2));

	while ((dllModule3 = GetModuleHandleA("GameDll7.dll")) == nullptr)
	{
		spdlog::warn("GameDll7.dll not loaded yet. Waiting...");
		Sleep(100);
	}

	spdlog::info("Successfully obtained handle for GameDll7.dll: 0x{:X}", reinterpret_cast<uintptr_t>(dllModule3));
...
void FOVFix()
{
	if (eGameType == Game::CBGH2007AA && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(dllModule2, "D9 85 D0 00 00 00 56 D8 0D ?? ?? ?? ??");
		if (CameraFOVInstructionScanResult)
		{
			spdlog::info("Camera FOV Instruction: Address is EngineDll7.dll+{:x}", CameraFOVInstructionScanResult - (std::uint8_t*)dllModule2);

			Memory::WriteNOPs(CameraFOVInstructionScanResult, 6);

			CameraFOVInstructionHook = safetyhook::create_mid(CameraFOVInstructionScanResult, [](SafetyHookContext& ctx)
			{
				float& fCurrentCameraFOV = Memory::ReadMem(ctx.ebp + 0xD0);
		...
		if (AspectRatioInstruction1ScanResult)
		{
			...
			{
				Memory::ReadMem(ctx.ebp + 0xD4) = 0.75f / fAspectRatioScale;
			});
		}
		else
		{
		...
		std::uint8_t* AspectRatioInstruction2ScanResult = Memory::PatternScan(dllModule2, "D8 B5 D8 00 00 00 D8 C9 D9 95 E0 00 00 00");
		if (AspectRatioInstruction2ScanResult)
		{
			...
			{
				Memory::ReadMem(ctx.ebp + 0xD8) = 1.0f;
			});
		}
		else
		{
