...
// Fix details
std::string sFixName = "MedalOfHonorPacificAssaultFOVFix";
...
constexpr float fOriginalHipfireFOV = 80.0f;
...
const std::map<Game, GameInfo> kGames = {
	{Game::MOHPA, {"Medal of Honor: Pacific Assault", "mohpa.exe"}},
};
...
void WeaponVFOVInstructionMidHook(SafetyHookContext& ctx)
{
	...
	_asm
	{
		fld dword ptr ds:[fNewWeaponVFOV]
	}
}

void FOVFix()
{
	if (eGameType == Game::MOHPA && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "D9 05 ?? ?? ?? ?? DB 05 ?? ?? ?? ?? DB 05");
		...
		std::uint8_t* HipfireCameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "68 ?? ?? ?? ?? 8B CE C7 44 24 ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B 44 24");
		if (HipfireCameraFOVInstructionScanResult)
		{
			...
			fNewHipfireFOV = fOriginalHipfireFOV * fFOVFactor;

			Memory::Write(HipfireCameraFOVInstructionScanResult + 1, fNewHipfireFOV);
		}
		else
		{
			spdlog::error("Failed to locate hipfire camera FOV instruction scan memory address.");
			return;
		}

		std::uint8_t* WeaponFOVInstructionsScanResult = Memory::PatternScan(exeModule, "D9 05 DC D8 ED 01 8B 48 1C 8B 15 F0 D4 ED 01 8B 42 1C 8B 15 5C D4 ED 01 51 8B 0D F8 D4 ED 01 D8 49 1C 50 83 EC 08 8D 84 24 C8 00 00 00 D8 0D F8 D9 BC 00 D9 5C 24 04 D9 05 DC D8 ED 01");
		if (WeaponFOVInstructionsScanResult)
		{
			...
			CameraFOV2Address = Memory::GetPointerFromAddress<uint32_t>(WeaponFOVInstructionsScanResult + 57, Memory::PointerMode::Absolute);
		...
		{
			spdlog::error("Failed to locate weapon FOV instruction scan memory address.");
			return;
		}
	}
}
