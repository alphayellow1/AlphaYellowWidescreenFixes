...
// Fix details
std::string sFixName = "WSCReal09WorldSnookerChampionshipFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::WSCR09WSC, {"WSC Real 09: World Snooker Championship", "WSC2009.exe"}},
};
...
void SetARAndFOV()
{
	std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "D9 05 ?? ?? ?? ?? 51 D9 1C 24 E8 ?? ?? ?? ?? D9 44 24 ?? 83 C4 ?? D9 9C 24 ?? ?? ?? ?? 89 84 24 ?? ?? ?? ?? D9 44 24 ?? D9 9C 24 ?? ?? ?? ?? D9 44 24 ?? D9 9C 24 ?? ?? ?? ?? D9 44 24 ?? D9 9C 24 ?? ?? ?? ?? D9 44 24 ?? D9 44 24 ?? D9 44 24 ?? D9 C0");
	...
	std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "D9 44 24 ?? 51 D9 15");
...
void FOVFix()
{
	if (eGameType == Game::WSCR09WSC && bFixActive == true)
	{
		std::uint8_t* ResolutionInstructionsScanResult = Memory::PatternScan(exeModule, "8B 54 24 ?? 89 90 ?? ?? ?? ?? 8B 54 24 ?? 89 90 ?? ?? ?? ?? 8B 54 24 ?? 89 90 ?? ?? ?? ?? 8B 54 24 ?? 83 C1");
