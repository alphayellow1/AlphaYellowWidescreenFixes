...
// Fix details
std::string sFixName = "TechDeckDudeBareKnuckleGrindWidescreenFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::TDDBKG, {"Tech Deck Dude: Bare Knuckle Grind", "techdeck_bkg.exe"}},
};
...
void WidescreenFix()
{
	if (eGameType == Game::TDDBKG && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* Resolution1ScanResult = Memory::PatternScan(exeModule, "C7 44 24 14 80 02 00 00 C7 44 24 18 E0 01 00 00");
		if (Resolution1ScanResult)
		{
			...
			Memory::Write(Resolution1ScanResult + 4, iCurrentResX);
		...
		std::uint8_t* Resolution2ScanResult = Memory::PatternScan(exeModule, "81 7C 24 08 80 02 00 00 75 0A 81 7C 24 0C E0 01 00 00");
		if (Resolution2ScanResult)
		{
			...
			Memory::Write(Resolution2ScanResult + 4, iCurrentResX);
		...
		std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "D8 0D ?? ?? ?? ?? D9 5C 24 18 8D 44 24 14 50 56 E8 ?? ?? ?? ?? 83 C4 08 8B CF 56");
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "D9 05 ?? ?? ?? ?? D8 0D ?? ?? ?? ?? 8B 7C 24 10 D8 0D ?? ?? ?? ?? 83 7F 08 02 D9 F2");
