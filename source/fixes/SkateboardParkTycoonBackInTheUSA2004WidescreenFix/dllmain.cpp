...
void WidescreenFix()
{
	if (eGameType == Game::SPTBITU2004 && bFixActive == true)
	{
		...
		std::uint8_t* ResolutionScanResult = Memory::PatternScan(exeModule, "C7 40 2C ?? ?? ?? ?? E8 F0 18 06 00 C7 40 30 ?? ?? ?? ?? C2 04 00 E8 E1 18 06 00 C7 40 2C ?? ?? ?? ?? E8 D5 18 06 00 C7 40 30 ?? ?? ?? ?? C2 04 00 E8 C6 18 06 00 C7 40 2C ?? ?? ?? ?? E8 BA 18 06 00 C7 40 30 ?? ?? ?? ?? C2 04 00");
