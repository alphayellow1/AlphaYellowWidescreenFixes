...
void FOVFix()
{
	if (eGameType == Game::COMM && bFixActive == true)
	{
		...
		{
			spdlog::info("Camera VFOV: Address is {:s}+{:x}", sExeName.c_str(), CameraVFOVScanResult + 0x4 - (std::uint8_t*)exeModule);

			Memory::Write(CameraVFOVScanResult + 0x4, fNewCameraFOV);
		}
		else
		{
