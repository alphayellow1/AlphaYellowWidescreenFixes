...
// Constants
const streampos kResolutionWidthOffset = 0x000C13DA;
...
void OpenFile(fstream &file)
{
    ...
    while (fileNotFound)
    {
        ...
        if (!file.is_open())
        {
            cout << "\nFailed to open lbt.exe, check if the executable has special permissions allowed that prevent the fixer from opening it (e.g: read-only mode), it's not present in the same directory as the fixer, or if the executable is currently running. Press Enter when all the mentioned problems are solved." << endl;
            do
            {
                ch = _getch(); // Waits for user to press a key
        ...
        {
            cout << "\nlbt.exe opened successfully!" << endl;
...
int main()
{
    cout << "The Land Before Time: Big Water Adventure (2002) Widescreen Fixer v1.0 by AlphaYellow, 2024\n\n----------------\n";
