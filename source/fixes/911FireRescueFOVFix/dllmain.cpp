...
class FireRescueFix final : public FixBase
{
public:
	explicit FireRescueFix(HMODULE selfModule) : FixBase(selfModule)
	{
		s_instance_ = this;
	}

	~FireRescueFix() override
	{
		if (s_instance_ == this)
		{
			s_instance_ = nullptr;
...
protected:
	...
	void ApplyFix() override
	{
		...
		auto CameraFOVScanResult = Memory::PatternScan(ExeModule(), "8B 5C 24 ?? 56 8B 74 24 ?? 89 30");
...
private:
	...
	inline static FireRescueFix* s_instance_ = nullptr;

	void CameraFOVMidHook(SafetyHookContext& ctx)
	{
		float& fCurrentCameraFOV = Memory::ReadMem(ctx.esp + 0x20);

		m_newCameraFOV = Maths::CalculateNewFOV_DegBased(fCurrentCameraFOV, m_aspectRatioScale) * m_fovFactor;

		ctx.ebx = std::bit_cast<uintptr_t>(m_newCameraFOV);
	}
};

static std::unique_ptr<FireRescueFix> g_fix;
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		g_fix = std::make_unique<FireRescueFix>(hModule);
