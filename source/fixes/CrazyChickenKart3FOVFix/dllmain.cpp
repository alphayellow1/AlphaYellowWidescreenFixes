...
void FOVFix()
{
	if (eGameType == Game::CCK3 && bFixActive == true)
	{
		...
		std::uint8_t* InARaceTriggerInstructionScanResult = Memory::PatternScan(exeModule, "89 1D ?? ?? ?? ?? 89 1D ?? ?? ?? ?? 74 26");
		...
		if (CameraHFOVInstructionScanResult)
		{
			...
			{
				float& fCurrentCameraHFOV = Memory::ReadMem(ctx.eax + 0x30C);
		...
		if (CameraVFOVInstructionScanResult)
		{
			...
			{
				float& fCurrentCameraVFOV = Memory::ReadMem(ctx.eax + 0x310);				
