...
// Fix details
std::string sFixName = "TerroristTakedownPaybackFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::TTP, {"Terrorist Takedown: Payback", "payback.exe"}},
};
...
{
	float& fCurrentCameraFOV = *reinterpret_cast<float*>(ctx.esi + 0x68C);
...
void FOVFix()
{
	if (eGameType == Game::TTP && bFixActive == true)
	{
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "D9 86 8C 06 00 00 D8 A6 94 06 00 00 DD 05 ?? ?? ?? ?? E8 ?? ?? ?? ??");
