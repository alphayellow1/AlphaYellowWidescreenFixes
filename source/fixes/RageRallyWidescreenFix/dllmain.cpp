...
// Fix details
std::string sFixName = "RageRallyWidescreenFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::RR_CONFIG, {"Rage Rally - Configuration", "cfg3dr2000.exe"}},
	{Game::RR_GAME, {"Rage Rally", "rally.exe"}},
};
...
void WidescreenFix()
{
	if (bFixActive == true)
	{
		if (eGameType == Game::RR_CONFIG)
		{
			std::uint8_t* ResolutionListUnlockScanResult = Memory::PatternScan(exeModule, "68 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 ?? 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 ?? 8B 45");
			if (ResolutionListUnlockScanResult)
			{
				...
				static uint32_t iMaxWidth = 30000;

				static uint32_t iMaxHeight = 30000;

				Memory::Write(ResolutionListUnlockScanResult + 1, &iMaxWidth);

				Memory::Write(ResolutionListUnlockScanResult + 19, &iMaxHeight);
			}
			else
			{
