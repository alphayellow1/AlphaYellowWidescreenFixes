...
// Fix details
std::string sFixName = "MistmareFOVFix";
...
void FOVFix()
{
	if (eGameType == Game::MM && bFixActive == true)
	{
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "8B 86 94 00 00 00 8B 8E 90 00 00 00 89 55 0C D8 25 6C F9 57 00");
		if (CameraFOVInstructionScanResult)
		{
			spdlog::info("Camera HFOV Instruction: Address is {:s}+{:x}", sExeName.c_str(), CameraFOVInstructionScanResult + 6 - (std::uint8_t*)exeModule);
			...
			{
				float& fCurrentCameraHFOV = *reinterpret_cast<float*>(ctx.esi + 0x90);

				float& fCurrentCameraVFOV = *reinterpret_cast<float*>(ctx.esi + 0x94);
			...
			{
				float& fCurrentCameraHFOV2 = *reinterpret_cast<float*>(ctx.esi + 0x90);

				float& fCurrentCameraVFOV2 = *reinterpret_cast<float*>(ctx.esi + 0x94);
