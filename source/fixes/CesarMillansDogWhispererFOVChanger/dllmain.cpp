...
// Fix details
std::string sFixName = "CesarMillansDogWhispererFOVChanger";
...
const std::map<Game, GameInfo> kGames = {
	{Game::CMDW, {"Cesar Millan's Dog Whisperer", "DogWhisperer.exe"}},
};
...
void FOVChanger()
{
	if (eGameType == Game::CMDW && bFixActive == true)
	{
		std::vector<std::uint8_t*> CameraFOVInstructionsScansResult = Memory::PatternScan(exeModule, "D9 05 ?? ?? ?? ?? D9 1C 24 E8 ?? ?? ?? ?? 8D AF ?? ?? ?? ?? 56 8B CD E8 ?? ?? ?? ?? D9 05 ?? ?? ?? ?? 8D 8F ?? ?? ?? ?? 89 8F ?? ?? ?? ?? D9 5D ?? D9 45 ?? 51 8B CD D9 1C 24 E8 ?? ?? ?? ?? D9 45 ?? DC 05 ?? ?? ?? ?? 51",
		"D9 05 ?? ?? ?? ?? D9 1C 24 E8 ?? ?? ?? ?? 8D AF ?? ?? ?? ?? 56 8B CD E8 ?? ?? ?? ?? D9 05 ?? ?? ?? ?? 8D 8F ?? ?? ?? ?? 89 8F ?? ?? ?? ?? D9 5D ?? D9 45 ?? 51 8B CD D9 1C 24 E8 ?? ?? ?? ?? D9 45 ?? DC 05 ?? ?? ?? ?? D9 5C 24",
		"D9 05 ?? ?? ?? ?? D9 1C 24 E8 ?? ?? ?? ?? D9 05");
		if (Memory::AreAllSignaturesValid(CameraFOVInstructionsScansResult) == true)
		{
			...
			CameraFOVAddress = Memory::GetPointerFromAddress(CameraFOVInstructionsScansResult[FOV1] + 2, Memory::PointerMode::Absolute);

			Memory::WriteNOPs(CameraFOVInstructionsScansResult, FOV1, FOV3, 0, 6);
