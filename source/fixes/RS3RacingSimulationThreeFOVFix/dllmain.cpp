...
// Fix details
std::string sFixName = "RS3RacingSimulationThreeFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::RS3, {"RS3: Racing Simulation Three", "RS3.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::RS3 && bFixActive == true)
	{
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "8B 56 ?? 50 51 8B 4E ?? 52 E8 ?? ?? ?? ?? 5E");
		...
		std::vector<std::uint8_t*> AspectRatioInstructionsScansResult = Memory::PatternScan(exeModule, "D9 46 ?? D8 76 ?? 8B 56", "8B 56 1C 8B 46 18 8B 4E 14");
		if (Memory::AreAllSignaturesValid(AspectRatioInstructionsScansResult) == true)
		{
			...
			spdlog::info("Races Aspect Ratio Instruction: Address is {:s}+{:x}", sExeName.c_str(), AspectRatioInstructionsScansResult[RacesAspectRatioScan] + 6 - (std::uint8_t*)exeModule);
			...
			Memory::PatchBytes(AspectRatioInstructionsScansResult[RacesAspectRatioScan] + 6, "\x90\x90\x90", 3);
			...
			RacesAspectRatioInstructionHook = safetyhook::create_mid(AspectRatioInstructionsScansResult[RacesAspectRatioScan] + 6, [](SafetyHookContext& ctx)
			{
				...
				ctx.ecx = std::bit_cast<uintptr_t>(fNewRacesAspectRatio);
			});
		}		
	}
}
