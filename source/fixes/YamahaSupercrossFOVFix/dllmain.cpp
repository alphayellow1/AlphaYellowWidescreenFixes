...
// Fix details
std::string sFixName = "YamahaSupercrossFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::YS, {"Yamaha Supercross", "Game.exe"}},
};
...
{
	float& fCurrentCameraFOV1 = Memory::ReadMem(ctx.esi + 0x2E8);
...
{
	float& fCurrentCameraFOV2 = Memory::ReadMem(ctx.ecx + 0x2E8);
...
void FOVFix()
{
	if (eGameType == Game::YS && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* AspectRatioInstruction1ScanResult = Memory::PatternScan(exeModule, "C7 81 1C 03 00 00 39 8E E3 3F C2 04 00 C7 81 1C 03 00 00 AB AA AA 3F");
		if (AspectRatioInstruction1ScanResult)
		{
			...
			Memory::Write(AspectRatioInstruction1ScanResult + 19, fNewAspectRatio);
		}
		else
		{
		...
		std::uint8_t* AspectRatioInstruction2ScanResult = Memory::PatternScan(exeModule, "8B 85 1C 03 00 00 8B 8B CC 00 00 00 8B 93 C8 00 00 00 50");
		...
		std::uint8_t* AspectRatioInstruction3ScanResult = Memory::PatternScan(exeModule, "D8 8E 1C 03 00 00 D9 1D ?? ?? ?? ?? D9 86 18 03 00 00 5E D8 0D ?? ?? ?? ?? D9 F2");
		if (AspectRatioInstruction3ScanResult)
		{
			...
			Memory::PatchBytes(AspectRatioInstruction3ScanResult, "\x90\x90\x90\x90\x90\x90", 6);			

			AspectRatioInstruction3Hook = safetyhook::create_mid(AspectRatioInstruction3ScanResult, AspectRatioInstruction3MidHook);
		}
		else
		{
			spdlog::error("Failed to locate aspect ratio instruction 3 scan memory address.");
			return;
		}
		...
		*/
		...
		std::uint8_t* CameraFOVInstruction3ScanResult = Memory::PatternScan(exeModule, "8B 85 18 03 00 00 51 52 8D B5 04 04 00 00 50 8B CE");
		if (CameraFOVInstruction3ScanResult)
		{
			...
			{
				float& fCurrentCameraFOV3 = Memory::ReadMem(ctx.ebp + 0x318);
