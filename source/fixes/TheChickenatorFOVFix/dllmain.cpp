...
// Fix details
std::string sFixName = "TheChickenatorFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::TC, {"The Chickenator", "chickenator.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::TC && bFixActive == true)
	{
		...
		std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "D9 05 ?? ?? ?? ?? 8B 10 D8 B1 20 01 00 00 89 54 24 10");
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "DC 3D ?? ?? ?? ?? D9 59 70 C2 04 00 90 90 90 90 90 90");
