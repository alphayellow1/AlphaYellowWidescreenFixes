...
class DoomsdayRacersFix final : public FixBase
{
public:
	explicit DoomsdayRacersFix(HMODULE selfModule) : FixBase(selfModule)
	{
		s_instance_ = this;
	}

	~DoomsdayRacersFix() override
	{
		if (s_instance_ == this)
		{
			s_instance_ = nullptr;
...
protected:
	...
	{
		return Util::stringcmp_caseless(exeName, "DoomsdayRacers.exe");
	}
	...
	void ApplyFix() override
	{
		...
		auto CameraFOVScanResult = Memory::PatternScan(ExeModule(), "C7 86 ?? ?? ?? ?? ?? ?? ?? ?? C7 86 ?? ?? ?? ?? ?? ?? ?? ?? 8B C6 5E 5D");
...
private:
	...
	void GetResolutionFromIni()
	{
		...
		std::ifstream iniFile("DoomsdayRacers.ini");
	...
	inline static DoomsdayRacersFix* s_instance_ = nullptr;
};

static std::unique_ptr<DoomsdayRacersFix> g_fix;
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		g_fix = std::make_unique<DoomsdayRacersFix>(hModule);
		g_fix->Start();
		break;
	}
