...
const streampos kHFOVOffset = 0x00096371;
const streampos kVFOVOffset = 0x0009638C;
...
void OpenFile(fstream &file)
{
    ...
    while (fileNotFound)
    {
        ...
        if (!file.is_open())
        {
            cout << "\nFailed to open client.exe, check if the DLL file has special permissions allowed that prevent the fixer from opening it (e.g: read-only mode), it's not present in the same directory as the fixer, or if the DLL is currently being used. Press Enter when all the mentioned problems are solved." << endl;
            do
            {
