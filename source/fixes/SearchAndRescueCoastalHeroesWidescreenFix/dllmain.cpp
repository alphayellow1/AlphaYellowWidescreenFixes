...
// Fix details
std::string sFixName = "SearchAndRescueCoastalHeroesWidescreenFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::SARCH, {"Search & Rescue: Coastal Heroes", "Sar4.exe"}},
};
...
void WidescreenFix()
{
	if (eGameType == Game::SARCH && bFixActive == true)
	{
		...
		std::uint8_t* Resolution1600x1200ScanResult = Memory::PatternScan(exeModule, "68 ?? ?? ?? ?? 6A ?? E8 ?? ?? ?? ?? 8B 0D ?? ?? ?? ?? 8B 15");
		...
		if (CameraFOVInstructionScanResult)
		{
			...
			{
				float& fCurrentCameraFOV = Memory::ReadMem(ctx.edx);
