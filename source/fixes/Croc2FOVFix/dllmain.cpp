...
class Croc2Fix final : public FixBase
{
public:
	explicit Croc2Fix(HMODULE selfModule) : FixBase(selfModule)
	{
		s_instance_ = this;
	}
...
protected:
	...
	{
		return Util::stringcmp_caseless(exeName, "Croc2.exe");
	}
	...
	{
		auto ResolutionScanResult = Memory::PatternScan(ExeModule(), "8B 4B ?? 8B 45 ?? 89 0D");
		...
		auto AspectRatioScanResult = Memory::PatternScan(ExeModule(), "D9 05 ?? ?? ?? ?? D8 35 ?? ?? ?? ?? D9 1D ?? ?? ?? ?? D9 05");
		...
		auto CameraFOVScanResult = Memory::PatternScan(ExeModule(), "68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 59 C3 90 90 8B 44 24");
...
private:
	...
	static constexpr uint32_t m_originalCameraFOV = 192;
	uint32_t m_newCameraFOV = 0;

	inline static Croc2Fix* s_instance_ = nullptr;
};

static std::unique_ptr<Croc2Fix> g_fix;
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		g_fix = std::make_unique<Croc2Fix>(hModule);
		g_fix->Start();
		break;
	}
