...
class DownhillSlalomFix final : public FixBase
{
public:
	explicit DownhillSlalomFix(HMODULE selfModule) : FixBase(selfModule)
	{
		s_instance_ = this;
	}

	~DownhillSlalomFix() override
	{
		if (s_instance_ == this)
		{
			s_instance_ = nullptr;
...
protected:
	...
	{
		return Util::stringcmp_caseless(exeName, "DownhillSlalom.exe");
	}
	...
	void ApplyFix() override
	{
		...
		auto CameraFOVScanResult = Memory::PatternScan(ExeModule(), "C7 86 ?? ?? ?? ?? ?? ?? ?? ?? C7 86 ?? ?? ?? ?? ?? ?? ?? ?? 8B C6");
...
private:
	...
	void GetResolutionFromIni()
	{
		...
		std::ifstream iniFile("DownhillSlalom.ini");
	...
	inline static DownhillSlalomFix* s_instance_ = nullptr;
};

static std::unique_ptr<DownhillSlalomFix> g_fix;
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		g_fix = std::make_unique<DownhillSlalomFix>(hModule);
		g_fix->Start();
		break;
	}
