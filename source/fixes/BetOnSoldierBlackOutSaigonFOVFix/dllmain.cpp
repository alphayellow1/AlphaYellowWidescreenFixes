...
bool DetectGame()
{
	...
	while ((dllModule3 = GetModuleHandleA("kte_dx9.dll")) == nullptr)
	{
		spdlog::warn("kte_dx9.dll not loaded yet. Waiting...");
		Sleep(100);
	}

	spdlog::info("Successfully obtained handle for kte_dx9.dll: 0x{:X}", reinterpret_cast<uintptr_t>(dllModule3));
...
{
	fNewCameraFOVMultiplier = fFOVFactor * CalculateNewFOVMultiplier(RadToDeg(fCurrentCameraFOV));
...
void FOVFix()
{
	if (eGameType == Game::BOSBOS && bFixActive == true)
	{
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(dllModule3, "8B 8E D4 00 00 00 52 50 51 8D 4C 24 2C FF 15 ?? ?? ?? ?? 8B 55 00 8D 44 24 1C 50 8B CD");
		if (CameraFOVInstructionScanResult)
		{
			spdlog::info("Camera FOV Instruction: Address is kte_dx9.dll+{:x}", CameraFOVInstructionScanResult - (std::uint8_t*)dllModule3);
			...
			{
				// Reference the current camera FOV located in the memory address [ESI+0xD4]
				float& fCurrentCameraFOV = *reinterpret_cast<float*>(ctx.esi + 0xD4);
				...
				if (fCurrentCameraFOV == 1.518436432f)
				{
