...
void FOVFix()
{
	if (eGameType == Game::SGEWC && bFixActive == true)
	{
		...
		std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "DC 0D ?? ?? ?? ?? D9 C9 DC 0D ?? ?? ?? ?? D9 C9 DD 05");
		if (AspectRatioInstructionScanResult)
		{
			...
			dNewAspectRatio = 1.5 / (double)fAspectRatioScale;
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "DD 05 ?? ?? ?? ?? D9 C9 D8 C9 D9 CA DE C9 D9 C9 D9 5D");
		if (CameraFOVInstructionScanResult)
		{
			...
			dNewCameraFOV = 0.5 / (double)fFOVFactor;
