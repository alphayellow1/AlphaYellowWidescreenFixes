...
const std::map<Game, GameInfo> kGames = {
	{Game::GMR, {"GM Rally", "GM Rally.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::GMR && bFixActive == true)
	{
		...
		std::uint8_t* InARaceTriggerInstructionScanResult = Memory::PatternScan(dllModule2, "39 1D ?? ?? ?? ?? 75 ?? 8B 0D");
		if (InARaceTriggerInstructionScanResult)
		{
			...
			uint8_t* TriggerValueAddress = Memory::GetPointerFromAddress(InARaceTriggerInstructionScanResult + 2, Memory::PointerMode::Absolute);
