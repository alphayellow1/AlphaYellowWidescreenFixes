...
// Fix details
std::string sFixName = "MashedFOVFix";
...
void FOVFix()
{
	if (eGameType == Game::MASHED && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* CameraHFOVAndVFOVInstructionScanResult = Memory::PatternScan(exeModule, "D8 0D ?? ?? ?? ?? D9 5C 24 00 D9 40 70 D8 48 58 D9 05 ?? ?? ?? ??");
