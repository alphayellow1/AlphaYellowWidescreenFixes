...
const streampos kFOVOffset = 0x00344126;

// Variables
int choice1, choice2, tempChoice;
uint32_t newWidth, newHeight, newCustomResolutionValue;
bool fileNotFound, validKeyPressed;
double oldWidth = 4.0, oldHeight = 3.0, oldHFOV = 75.0, oldAspectRatio = oldWidth / oldHeight, newAspectRatio;
...
void OpenFile(fstream &file)
{
    ...
    file.open("EngineDll7r.dll", ios::in | ios::out | ios::binary);
    ...
    while (fileNotFound)
    {
        ...
        if (!file.is_open())
        {
            cout << "\nFailed to open EngineDll7r.dll, check if the DLL has special permissions allowed that prevent the fixer from opening it (e.g: read-only mode), it's not present in the same directory as the fixer, or if it's currently being used. Press Enter when all the mentioned problems are solved." << endl;
            do
            {
        ...
        {
            cout << "\nEngineDll7r.dll opened successfully!" << endl;
...
int main()
{
    ...
    cout << "Harley-Davidson: Race to the Rally (2006) FOV Fixer v1.0 by AlphaYellow, 2024\n\n----------------\n";
    ...
    do
    {
        ...
        case 2:
            cout << "\n- Enter the desired FOV (in degrees, default for 4:3 aspect ratio is 75\u00B0): ";
