...
void WidescreenFix()
{
	if (eGameType == Game::PR && bFixActive == true)
	{
		...
		{
			spdlog::info("Resolution Width Instruction: Address is ChromeEngine.dll+{:x}", ResolutionWidthInstructionScanResult + 0x12 - (std::uint8_t*)dllModule2);

			static SafetyHookMid ResolutionWidthInstructionMidHook{};

			ResolutionWidthInstructionMidHook = safetyhook::create_mid(ResolutionWidthInstructionScanResult + 0x12, [](SafetyHookContext& ctx)
			{
