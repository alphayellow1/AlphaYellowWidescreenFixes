...
// Fix details
std::string sFixName = "SupremeSnowboardingFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::SS, {"Supreme Snowboarding", "Supreme.exe"}},
	{Game::BZ, {"Boarder Zone", "BoarderZ.exe"}},
};
...
bool DetectGame()
{
	while (!dllModule2)
	{
		...
		spdlog::warn("Waiting for Supreme_Game.dll to load...");
		Sleep(1000); // Delay to wait for the DLL to load
	}

	spdlog::info("Successfully obtained handle for Supreme_Game.dll: 0x{:X}", reinterpret_cast<uintptr_t>(dllModule2));
...
void FOVFix()
{
	if (bFixActive == true)
	{
		...
		if (CameraHFOVInstructionScanResult)
		{
			...
			{
				Memory::ReadMem(ctx.esp + 0x2D0) = fNewCameraHFOV;
			});
		}
		else
		{
