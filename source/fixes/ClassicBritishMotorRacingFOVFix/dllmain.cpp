...
// Fix details
std::string sFixName = "ClassicBritishMotorRacingFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::CBMR, {"Classic British Motor Racing", "ClassicBritishMotorRacing.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::CBMR && bFixActive == true)
	{
		...
		if (CameraHFOVInstructionScanResult)
		{
			...
			Memory::WriteNOPs(CameraHFOVInstructionScanResult, 3);
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "D9 46 18 D9 F2 66 8B 46 10 66 8B 4E 12 89 44 24 10 8B 46 04 8D 54 24 04 52 50");
		if (CameraFOVInstructionScanResult)
		{
			...
			{
				fCurrentCameraFOV = *reinterpret_cast<float*>(ctx.esi + 0x18);
			});
		}
		else
		{
		...
		std::uint8_t* CameraFOVInstruction2ScanResult = Memory::PatternScan(exeModule, "D9 44 24 08 8B 44 24 04 50");
