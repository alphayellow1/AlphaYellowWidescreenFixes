...
// Fix details
std::string sFixName = "WantedGunsWidescreenFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::WG, {"Wanted Guns", "WantedGuns.exe"}},
};
...
void WidescreenFix()
{
	if (eGameType == Game::WG && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		std::uint8_t* ResolutionInstructions1ScanResult = Memory::PatternScan(exeModule, "89 15 ?? ?? ?? ?? A3 ?? ?? ?? ?? E8 ?? ?? ?? ?? 50");
		if (ResolutionInstructions1ScanResult)
		{
			...
			ResolutionWidth1Address = Memory::GetPointerFromAddress(ResolutionInstructions1ScanResult + 2, Memory::PointerMode::Absolute);

			ResolutionHeight1Address = Memory::GetPointerFromAddress(ResolutionInstructions1ScanResult + 7, Memory::PointerMode::Absolute);
		...
		std::uint8_t* ResolutionInstructions2ScanResult = Memory::PatternScan(exeModule, "89 81 A0 00 00 00 89 91 A4 00 00 00 C2 08 00 90 90 90 90 90 90 90 90 90");
		if (ResolutionInstructions2ScanResult)
		{
			...
			{
				Memory::ReadMem(ctx.ecx + 0xA0) = iCurrentResX;

				Memory::ReadMem(ctx.ecx + 0xA4) = iCurrentResY;
			});
		}
		else
		{
		...
		std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "8B 56 34 8D 9E 84 00 00 00 53 50");
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "8B 46 38 51 52 50");
		if (CameraFOVInstructionScanResult)
		{
			...
			{
				float& fCurrentCameraFOV = Memory::ReadMem(ctx.esi + 0x38);
