...
void WidescreenFix()
{
	if (eGameType == Game::WG && bFixActive == true)
	{
		...
		if (ResolutionInstructions1ScanResult)
		{
			...
			ResolutionWidth1Address = Memory::GetPointerFromAddress<uint32_t>(ResolutionInstructions1ScanResult + 2, Memory::PointerMode::Absolute);

			ResolutionHeight1Address = Memory::GetPointerFromAddress<uint32_t>(ResolutionInstructions1ScanResult + 7, Memory::PointerMode::Absolute);
		...
		if (ResolutionInstructions2ScanResult)
		{
			...
			{
				*reinterpret_cast<int*>(ctx.ecx + 0xA0) = iCurrentResX;

				*reinterpret_cast<int*>(ctx.ecx + 0xA4) = iCurrentResY;
			});
		}
		else
		{
