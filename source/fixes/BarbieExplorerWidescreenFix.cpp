...
const streampos kHFOVOffset = 0x0008E5C4;
...
void OpenFile(fstream &file)
{
    ...
    file.open("Barbiex.exe", ios::in | ios::out | ios::binary);
    ...
    while (fileNotFound)
    {
        ...
        if (!file.is_open())
        {
            cout << "\nFailed to open Barbiex.exe, check if the executable has special permissions allowed that prevent the fixer from opening it (e.g: read-only mode), it's not present in the same directory as the fixer, or if it's currently running. Press Enter when all the mentioned problems are solved." << endl;
            do
            {
                ch = _getch(); // Waits for user to press a key
        ...
        {
            cout << "\nBarbiex.exe opened successfully!" << endl;
...
int main()
{
    cout << "Barbie Explorer (2002) Widescreen Fixer v1.0 by AlphaYellow, 2024\n\n----------------\n";
