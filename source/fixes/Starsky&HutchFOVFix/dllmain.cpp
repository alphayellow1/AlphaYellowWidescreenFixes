...
protected:
	...
	{
		m_resolutionScanResult = Memory::PatternScan(ExeModule(), "8B 02 A3 ?? ?? ?? ?? 8B 42");
		...
		m_cameraFOVScansResult = Memory::PatternScan(ExeModule(), "D8 0D ?? ?? ?? ?? D9 55 ?? D8 35 ?? ?? ?? ?? 51",
		...
		if (Memory::AreAllSignaturesValid(m_cameraFOVScansResult) == true)
		{
			...
			m_cameraFOV1Address = Memory::GetPointerFromAddress(m_cameraFOVScansResult[FOV1] + 2, Memory::PointerMode::Absolute);
			...
			Memory::Write(m_cameraFOVScansResult[FOV2] + 6, m_newCameraFOV2);
		}
	}
