...
const std::map<Game, GameInfo> kGames = {
	{Game::DINOSAUR, {"Disney's Dinosaur", "Dinosaur.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::DINOSAUR && bFixActive == true)
	{
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "8B 54 24 ?? 8B 44 24 ?? 8B 4C 24 ?? 52 8B 54 24 ?? 50 51 52 55");
		if (CameraFOVInstructionScanResult)
		{
			...
			{
				float& fCurrentCameraFOV = Memory::ReadMem(ctx.esp + 0x24);
