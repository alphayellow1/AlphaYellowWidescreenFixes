...
void FOVFix()
{
	if (eGameType == Game::CDH2005S && bFixActive == true)
	{
		...
		if (InsideCarTriggerInstructionScanResult)
		{
			...
			InsideCarValueAddress = Memory::GetPointerFromAddress(InsideCarTriggerInstructionScanResult + 1, Memory::PointerMode::Absolute);			
		...
		if (AspectRatioInstructionsScanResult)
		{
			...
			{
				Memory::ReadMem(ctx.ebx + 0xD4) = 0.75f / (fNewAspectRatio / fOldAspectRatio);
			});
