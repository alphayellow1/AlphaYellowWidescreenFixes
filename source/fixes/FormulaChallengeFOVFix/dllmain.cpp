...
// Fix details
std::string sFixName = "FormulaChallengeFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::FC, {"Formula Challenge", "fcarsPC2.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::FC && bFixActive == true)
	{
		...
		std::vector<std::uint8_t*> CameraFOVInstructionsScansResult = Memory::PatternScan(exeModule, "8B 57 ?? 6A ?? 52 B9", "8B 44 24 ?? 56 50 B9");
		...
		if (Memory::AreAllSignaturesValid(AspectRatioInstructionsScansResult) == true)
		{
			...
			Memory::WriteNOPs(AspectRatioInstructionsScansResult[AR1], 7);
		...
		if (Memory::AreAllSignaturesValid(CameraFOVInstructionsScansResult) == true)
		{
			...
			{
				CameraFOVInstructionsMidHook(ctx.edi + 0x24, ctx.edx, fFOVFactor);
			});
			...
			{
				CameraFOVInstructionsMidHook(ctx.esp + 0x14, ctx.eax);
			});
		}
	}
}
