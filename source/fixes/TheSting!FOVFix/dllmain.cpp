...
void FOVFix()
{
	if ((eGameType == Game::TS_ENG || eGameType == Game::TS_RU || eGameType == Game::TS_GER) && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "D8 4C 24 ?? D9 5C 24 ?? E8 ?? ?? ?? ?? DD 5C 24");
		if (AspectRatioInstructionScanResult)
		{
			...
			{
				float& fCurrentAspectRatio = *reinterpret_cast<float*>(ctx.esp + 0x34);
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "D9 44 24 ?? D8 0D ?? ?? ?? ?? 53 56");
