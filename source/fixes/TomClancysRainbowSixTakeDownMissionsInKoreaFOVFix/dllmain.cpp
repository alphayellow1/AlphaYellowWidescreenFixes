...
// Fix details
std::string sFixName = "TomClancysRainbowSixTakeDownMissionsInKoreaFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::TCRSTDMIK, {"Tom Clancy's Rainbow Six: Take-Down - Missions in Korea", "TakeDown.exe"}},
...
{
	fNewAspectRatio2 = 1.805041909f * fAspectRatioScale;
...
void FOVFix()
{
	if (eGameType == Game::TCRSTDMIK && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "D8 73 78 89 83 80 00 00 00 D9 93 84 00 00 00 D9 E0 D9 9B 88 00 00 00 51");
		...
		if (CameraFOVInstructionScanResult)
		{
			...
			CameraFOVInstructionMidHook = safetyhook::create_mid(CameraFOVInstructionScanResult, [](SafetyHookContext& ctx)
			{
				...
				Memory::ReadMem(ctx.eax + 0x74) = fNewCameraFOV;
			});
		}
		else
		{
