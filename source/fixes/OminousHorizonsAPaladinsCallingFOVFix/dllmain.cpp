...
// Fix details
std::string sFixName = "OminousHorizonsAPaladinsCallingFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::OH, {"Ominous Horizons: A Paladin's Calling", "ominous.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::OH && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "A1 ?? ?? ?? ?? 8B 0D ?? ?? ?? ?? 50 E8 0E EE FF FF");
