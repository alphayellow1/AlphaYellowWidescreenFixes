...
void FOV()
{
	if (eGameType == Game::B2) {
		std::uint8_t* B2_HipfireAndCutscenesHFOVScanResult = Memory::PatternScan(exeModule, "8B B0 38 01 00 00 89 B4 24 D0 00 00 00");
		if (B2_HipfireAndCutscenesHFOVScanResult) {
			spdlog::info("Hipfire and Cutscenes HFOV: Address is {:s}+{:x}", sExeName.c_str(), B2_HipfireAndCutscenesHFOVScanResult - (std::uint8_t*)exeModule);
			static SafetyHookMid B2_HipfireAndCutscenesHFOVMidHook{};
			B2_HipfireAndCutscenesHFOVMidHook = safetyhook::create_mid(B2_HipfireAndCutscenesHFOVScanResult,
				[](SafetyHookContext& ctx) {
				*reinterpret_cast<float*>(ctx.eax + 0x138) = 2.0f * atanf(tanf(1.5707950592041016f / 2.0f) * ((static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY)) / oldAspectRatio));
			});
		}

		std::uint8_t* B2_HipfireAndCutscenesVFOVScanResult = Memory::PatternScan(exeModule, "8B B0 3C 01 00 00 89 B4 24 D4 00 00 00");
		if (B2_HipfireAndCutscenesVFOVScanResult) {
			spdlog::info("Hipfire and Cutscenes HFOV: Address is {:s}+{:x}", sExeName.c_str(), B2_HipfireAndCutscenesVFOVScanResult - (std::uint8_t*)exeModule);
			static SafetyHookMid B2_HipfireAndCutscenesVFOVMidHook{};
			B2_HipfireAndCutscenesVFOVMidHook = safetyhook::create_mid(B2_HipfireAndCutscenesVFOVScanResult,
				[](SafetyHookContext& ctx) {
		...
		/*std::uint8_t* B2_HipfireAndCutscenesVFOVScanResult = Memory::PatternScan(exeModule, "8B B0 3C 01 00 00 89 B4 24 D4 00 00 00");
		if (B2_HipfireAndCutscenesVFOVScanResult) {
			spdlog::info("Hipfire and Cutscenes VFOV: Address is {:s}+{:x}", sExeName.c_str(), B2_HipfireAndCutscenesVFOVScanResult - (std::uint8_t*)exeModule);
			static SafetyHookMid B2_HipfireAndCutscenesVFOVMidHook{};
			B2_HipfireAndCutscenesVFOVMidHook = safetyhook::create_mid(B2_HipfireAndCutscenesVFOVScanResult,
				[](SafetyHookContext& ctx) {
