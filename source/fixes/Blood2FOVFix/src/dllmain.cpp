...
void FOVFix()
{
	...
	{
		std::uint8_t* HipfireAndCutscenesHFOVScanResult = Memory::PatternScan(exeModule, "8B B0 38 01 00 00 89 B4 24 D0 00 00 00");
		if (HipfireAndCutscenesHFOVScanResult)
		{
			spdlog::info("Hipfire and Cutscenes HFOV: Address is {:s}+{:x}", sExeName.c_str(), HipfireAndCutscenesHFOVScanResult - (std::uint8_t*)exeModule);

			static SafetyHookMid HipfireAndCutscenesHFOVMidHook{};
			HipfireAndCutscenesHFOVMidHook = safetyhook::create_mid(HipfireAndCutscenesHFOVScanResult, [](SafetyHookContext& ctx)
			{
				if (*reinterpret_cast<float*>(ctx.eax + 0x138) == 1.5707950592041016f)
				{
					*reinterpret_cast<float*>(ctx.eax + 0x138) = 2.0f * atanf(tanf(1.5707950592041016f / 2.0f) * ((static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY)) / fOldAspectRatio));
				}
			});
		}
		...
		{
			spdlog::info("Hipfire and Cutscenes HFOV: Address is {:s}+{:x}", sExeName.c_str(), HipfireAndCutscenesVFOVScanResult - (std::uint8_t*)exeModule);
