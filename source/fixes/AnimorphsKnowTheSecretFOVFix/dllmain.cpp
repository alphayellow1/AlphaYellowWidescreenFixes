...
// Fix details
std::string sFixName = "AnimorphsKnowTheSecretFOVFix";
...
void FOVFix()
{
	if (eGameType == Game::AKTS && bFixActive == true)
	{
		...
		{
			spdlog::info("Aspect Ratio and Camera FOV Instructions Scan: Address is {:s}+{:x}", sExeName.c_str(), AspectRatioAndCameraFOVInstructionsScanResult - (std::uint8_t*)dllModule);

			static SafetyHookMid AspectRatioAndFOVInstructionsMidHook{};

			AspectRatioAndFOVInstructionsMidHook = safetyhook::create_mid(AspectRatioAndCameraFOVInstructionsScanResult, [](SafetyHookContext& ctx)
			{
