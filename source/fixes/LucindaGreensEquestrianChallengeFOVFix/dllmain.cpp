...
// Fix details
std::string sFixName = "LucindaGreensEquestrianChallengeFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::LGEC, {"Lucinda Green's Equestrian Challenge", "EquestrianChallenge.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::LGEC && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "89 15 ?? ?? ?? ?? 8B 51 08 A3 ?? ?? ?? ?? 8B 41 0C");
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "89 15 ?? ?? ?? ?? A3 ?? ?? ?? ?? 89 0D ?? ?? ?? ?? E9 ?? ?? ?? ?? CC");
