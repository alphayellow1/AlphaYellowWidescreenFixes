...
// Fix details
std::string sFixName = "JetFighterVHomelandProtectorFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::JFV, {"JetFighter V: Homeland Protector", "Game.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::JFV && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iCurrentResX) / static_cast<float>(iCurrentResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;
		
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "D9 44 24 ?? 56 D8 0D");
