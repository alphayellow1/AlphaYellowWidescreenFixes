	...
	if (eGameType == Game::CTEOI && bFixActive == true)
	{
		...
		if (CameraHFOVInstructionScanResult)
		{
			...
			CameraHFOVInstructionMidHook = safetyhook::create_mid(CameraHFOVInstructionScanResult, [](SafetyHookContext& ctx)
			{
				...
				float fModifiedHFOVValue = CalculateNewFOV(fCurrentCameraHFOV);
		...
		if (CameraVFOVInstructionScanResult)
		{
			...
			CameraVFOVInstructionMidHook = safetyhook::create_mid(CameraVFOVInstructionScanResult, [](SafetyHookContext& ctx)
			{
				...
				// Skip processing if this VFOV has already been computed
				if (std::find(vComputedVFOVs.begin(), vComputedVFOVs.end(), fCurrentCameraVFOV) != vComputedVFOVs.end())
				{
					return;
				}

				// Initializes the modified value to the current VFOV
				float fModifiedVFOVValue = fCurrentCameraHFOV / fNewAspectRatio;
				...
				// Records the computed VFOV to avoid recalculating it in future calls
				vComputedVFOVs.push_back(fModifiedVFOVValue);

				// Updates the ECX register with the new VFOV value
				ctx.edx = std::bit_cast<uintptr_t>(fModifiedVFOVValue);
			});
		}
		else
		{
