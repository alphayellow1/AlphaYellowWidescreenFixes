...
// Fix details
std::string sFixName = "StartopiaFOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::STARTOPIA, {"Startopia", "startopia.exe"}},
};
...
void FOVFix()
{
	if (eGameType == Game::STARTOPIA && bFixActive == true)
	{
		fNewAspectRatio = static_cast<float>(iNewResX) / static_cast<float>(iNewResY);

		fAspectRatioScale = fNewAspectRatio / fOldAspectRatio;

		std::uint8_t* AspectRatioInstructionScanResult = Memory::PatternScan(exeModule, "D8 0D ?? ?? ?? ?? D9 5D ?? D9 05 ?? ?? ?? ?? D8 75 ?? 8B 4D");
		...
		std::uint8_t* CameraFOVInstructionScanResult = Memory::PatternScan(exeModule, "D9 05 ?? ?? ?? ?? D8 0D ?? ?? ?? ?? 8B C1 56");
