...
// Fix details
std::string sFixName = "Cricket2004FOVFix";
...
const std::map<Game, GameInfo> kGames = {
	{Game::CRICKET2004, {"Cricket 2004", "Cricket2004.exe"}},
};
...
void CameraFOVInstructionsMidHook(uint8_t* FOVAddress, float fARScale, float fovFactor)
{
...
void FOVFix()
{
	if (eGameType == Game::CRICKET2004 && bFixActive == true)
	{
		...
		if (Memory::AreAllSignaturesValid(CameraFOVInstructionsScansResult) == true)
		{
			...
			CameraFOV1Address = Memory::GetPointerFromAddress<uint32_t>(CameraFOVInstructionsScansResult[FOV1] + 2, Memory::PointerMode::Absolute);

			CameraFOV2Address = Memory::GetPointerFromAddress<uint32_t>(CameraFOVInstructionsScansResult[FOV2] + 2, Memory::PointerMode::Absolute);
			...
			{
				CameraFOVInstructionsMidHook(CameraFOV1Address, 1.0f / fAspectRatioScale, 1.0f / fFOVFactor);
			});
			...
			{
				CameraFOVInstructionsMidHook(CameraFOV2Address, fAspectRatioScale, fFOVFactor);
			});
		}
	}
}
